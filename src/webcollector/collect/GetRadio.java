 ...
 * @author wqn
 */
public class GetRadio extends BreadthCrawler {
	 ...
	 */
	public GetRadio(String crawlPath, boolean autoParse) throws FileNotFoundException {
		super(crawlPath, autoParse);
		// 逗号进行分割，字符编码为UTF-8
		this.r = new CsvWriter("Radiolist.csv", ',', Charset.forName("UTF-8"));		
	}
	@Override
	public void visit(Page page, CrawlDatums next) {
		...
		// 网易云音乐song页面URL地址正则
		String song_regex = "^http://music.163.com/program\\?id=[0-9]+";
		// 创建Pattern对象
		Pattern songIdPattern = Pattern.compile("^http://music.163.com/program\\?id=([0-9]+)");
		...
		if (Pattern.matches(song_regex, page.getUrl())) {
			...
			String[] contents = {songId, songName, songSinger, url};

			Radio m = new Radio();
			m.setId(songId.toString());
			m.setRadioer(songName);
			m.setRadioName(songSinger);
			m.setUrl(url);
...
//			n++;		
	...
	public void test(Radio m){  
       ...
       public static void listStudents( ){
 	      ...
 	      try{
 	         ...
 	         List employees = session.createQuery("FROM Radio").list(); 
	...
	public static void go() throws Exception {
		...
		GetRadio crawler = new GetRadio("crawler", true);
